...
export default function Jobs() {
  ...
  const handleFilterChange = (category: string, value: string) => {
    setFilters((prev) => {
      ...
      console.log(newFilters); // Debugging the filter state
      return { ...prev, [category]: newFilters };
    });
  };
  ...
  useEffect(() => {
    console.log("Recalculating filteredRecentJobs");
    setFilteredRecentJobs(filteredJobs(recentJobs));
  }, [filters]); // Remove `recentJobs` to prevent unnecessary re-renders

  const currentJobs = tab === 1 ? filteredJobs(jobData) : filteredRecentJobs;
  console.log("Current Jobs:", currentJobs); // Debugging final output
